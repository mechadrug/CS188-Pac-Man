...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for i in all_coords:
        pacphysics_sentences.append(PropSymbolExpr(wall_str,i[0],i[1])>> ~PropSymbolExpr(pacman_str,i[0],i[1],time=t))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    KB.append(pacphysicsAxioms(1,all_coords,non_outer_wall_coords,walls_grid,None
                               ,allLegalSuccessorAxioms))
    # KB.append(pacphysicsAxioms(2,all_coords,non_outer_wall_coords,walls_grid,None,allLegalSuccessorAxioms))
...
def positionLogicPlan(problem) -> List:
    ...
    for i in range(50):
        ...
        print(i)
        KB.append(exactlyOne([PropSymbolExpr(pacman_str,t[0],t[1],time=i)for t in non_wall_coords]))
...
def foodLogicPlan(problem) -> List:
    ...
    for i in range(50):
        ...
        for x,y in food:
            KB.append((~PropSymbolExpr(pacman_str,x,y,time=i)& PropSymbolExpr(food_str,x,y,time=i))>>PropSymbolExpr(food_str,x,y,time=i+1))#如果pacman不在这个位置但这个位置有食物,那么对于当前来说下一个时间点也有食物
            KB.append((PropSymbolExpr(pacman_str,x,y,time=i)& PropSymbolExpr(food_str,x,y,time=i))>>~PropSymbolExpr(food_str,x,y,time=i+1))
    return None
    "*** BEGIN YOUR CODE HERE ***"
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
