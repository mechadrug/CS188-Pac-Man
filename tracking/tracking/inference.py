...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        Useful functions:
        BayesNet.getAllCPTsWithEvidence获取所有条件概率表,根据字典进行条件化
        normalize归一化,输入一个factor,传入一个概率之和为1的newfactor
        eliminate消除变量,将一个因子中某个变量求和,得到新的因子
        joinFactorsByVariable在某个变量上合并因子.
        joinFactors将多个因子合并
        """
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            ...
            prior_prob=self.beliefs[position]
            new_beliefs[position]=prior_prob*observation_prob
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # # print("type:",type(self.particles[0]))
        # for particle in self.particles:
        ...
        # total_weight=weight_distribution.total()
        # if total_weight==0:
        #     self.initializeUniformly(gameState)
        #     return
